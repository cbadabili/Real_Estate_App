...
router.get('/services', async (req, res) => {
    try {
        ...
        const sectionServices = services[section] || [];
...
async function getProviders(filters) {
    ...
    if (filters.location) {
        allProviders = allProviders.filter(p => p.service_area.toLowerCase().includes(filters.location.toLowerCase()));
    }
    if (filters.search) {
        allProviders = allProviders.filter(p => p.business_name.toLowerCase().includes(filters.search.toLowerCase()) ||
 ...
 */
async function getProviderReviews(providerId, opts = {}) {
...
async function getTrainingProviders(filters) {
    try {
        ...
        if (filters && filters.category && filters.category !== 'all' && typeof filters.category === 'string') {
            filtered = filtered.filter(provider => provider.category && provider.category.toLowerCase().includes(filters.category.toLowerCase()));
        }
        if (filters && filters.location && typeof filters.location === 'string') {
            filtered = filtered.filter(provider => provider.location && provider.location.toLowerCase().includes(filters.location.toLowerCase()));
        }
        if (filters && filters.search && typeof filters.search === 'string') {
            filtered = filtered.filter(provider => (provider.name && provider.name.toLowerCase().includes(filters.search.toLowerCase())) ||
                ...
                (provider.courses && Array.isArray(provider.courses) &&
                    provider.courses.some((course) => course && course.toLowerCase().includes(filters.search.toLowerCase()))));
        }
