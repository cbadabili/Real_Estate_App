...
export function registerLocationRoutes(app) {
    ...
    app.get("/api/locations/search", async (req, res) => {
        try {
            ...
            const type = req.query.type; // 'district', 'settlement', 'ward', 'plot'
            ...
            if (!type || type === 'district') {
                results.districts = await db
                    .select()
                    .from(districts)
                    .where(or(ilike(districts.name, searchPattern), ilike(districts.code, searchPattern)))
                    .orderBy(asc(districts.name))
                    .limit(limit);
            }
            ...
            if (!type || type === 'plot') {
                results.plots = await db
                    ...
                    .innerJoin(districts, eq(settlements.district_id, districts.id))
                    .where(or(ilike(plots.full_address, searchPattern), ilike(plots.street_name, searchPattern), ilike(plots.block_name, searchPattern)))
                    .orderBy(asc(plots.full_address))
                    .limit(limit);
            }
    ...
    app.get("/api/locations/suggestions", async (req, res) => {
        try {
            ...
            const suggestions = settlementSuggestions.map(s => ({
                ...s,
                level: 'settlement'
            }));
