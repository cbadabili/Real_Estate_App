...
import { authenticate, optionalAuthenticate, authorize, requireAdmin, requireModerator, AuthService } from "./auth-middleware";
...
export async function registerRoutes(app) {
    ...
    app.get("/api/properties", async (req, res) => {
        try {
            const filters = {
                ...
                minBathrooms: req.query.minBathrooms ? parseFloat(req.query.minBathrooms) : undefined,
                minSquareFeet: req.query.minSquareFeet ? parseInt(req.query.minSquareFeet) : undefined,
                maxSquareFeet: req.query.maxSquareFeet ? parseInt(req.query.maxSquareFeet) : undefined,
                city: req.query.city,
    ...
    app.post("/api/properties", async (req, res) => {
        try {
            const { areaText, placeName, placeId, latitude, longitude, locationSource, ...rest } = req.body;
            // Validate coordinates are within Botswana bounds
            const isNum = (v) => typeof v === "number" && Number.isFinite(v);
            const inBW = (lng, lat) => lng >= 20 && lng <= 29 && lat >= -27 && lat <= -17;
    ...
    app.get("/api/reviews", optionalAuthenticate, async (req, res) => {
        try {
            const filters = {
                revieweeId: req.query.reviewee_id ? parseInt(req.query.reviewee_id) : undefined,
                reviewerId: req.query.reviewer_id ? parseInt(req.query.reviewer_id) : undefined,
                reviewType: req.query.review_type,
                minRating: req.query.min_rating ? parseInt(req.query.min_rating) : undefined,
                maxRating: req.query.max_rating ? parseInt(req.query.max_rating) : undefined,
                status: req.query.status || 'active',
    ...
    app.get("/api/admin/dashboard", authenticate, requireAdmin, async (req, res) => {
        try {
            const [flaggedReviews, recentAuditLogs, userStats] = await Promise.all([
    ...
    app.get("/api/admin/users", authenticate, requireAdmin, async (req, res) => {
        try {
            ...
            if (search) {
                ...
                users = users.filter(user => user.firstName.toLowerCase().includes(searchLower) ||
    ...
    app.get("/api/admin/audit-log", authenticate, requireAdmin, async (req, res) => {
        try {
            const filters = {
                adminId: req.query.admin_id ? parseInt(req.query.admin_id) : undefined,
