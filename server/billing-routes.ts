  ...
  try {
    ...
    const [stats] = await db
      .select({
        ...
        pending_payments: sql<number>`SUM(CASE WHEN status = 'pending' THEN 1 ELSE 0 END)`,
        approved_payments: sql<number>`SUM(CASE WHEN status = 'succeeded' THEN 1 ELSE 0 END)`,
        rejected_payments: sql<number>`SUM(CASE WHEN status = 'failed' THEN 1 ELSE 0 END)`,
        total_revenue: sql<number>`SUM(CASE WHEN status = 'succeeded' THEN amount_bwp ELSE 0 END)`,
        pending_revenue: sql<number>`SUM(CASE WHEN status = 'pending' THEN amount_bwp ELSE 0 END)`
      })
      .from(payments);
    ...
    const [subscriptionStats] = await db
      .select({
        active_subscriptions: sql<number>`COUNT(*)`
      })
      .from(subscriptions)
    ...
    const planDistribution = await db
      ...
      .leftJoin(subscriptions, and(
        eq(subscriptions.plan_id, plans.id),
        eq(subscriptions.status, 'active')
      ))
