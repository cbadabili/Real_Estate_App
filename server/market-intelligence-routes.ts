  ...
  app.get('/api/market-intelligence/neighborhoods', async (req, res) => {
    try {
      ...
      const neighborhoodData = Object.values(neighborhoods).map((area: any) => {
        const totalValue = area.properties.reduce((sum: number, p: any) => sum + (p.price || 0), 0);
        return {
          name: area.name,
          averagePrice: Math.round(totalValue / area.properties.length),
          totalListings: area.properties.length,
          priceRange: {
            min: Math.min(...area.properties.map((p: any) => p.price || 0)),
            max: Math.max(...area.properties.map((p: any) => p.price || 0))
          }
        };
