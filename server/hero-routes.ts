...
router.get('/', async (req, res) => {
  try {
    ...
    const heroListings = await db
      ...
      .where(and(
        eq(hero_slots.is_active, true),
        lte(hero_slots.starts_at, now),
        gte(hero_slots.ends_at, now)
      ))
...
router.post('/properties/:propertyId/hero', authenticate, async (req, res) => {
  try {
    ...
    const [heroEntitlement] = await db
      ...
      .from(entitlements)
      .where(and(
        eq(entitlements.user_id, req.user.id),
        eq(entitlements.feature_key, 'heroSlots')
      ));
    ...
    const [existingSlot] = await db
      ...
      .from(hero_slots)
      .where(and(
        eq(hero_slots.property_id, propertyId),
        eq(hero_slots.is_active, true),
        lte(hero_slots.starts_at, now),
        gte(hero_slots.ends_at, now)
      ));
