...
-- Bathrooms should preserve half-baths; store as NUMERIC(3,1)
ALTER TABLE properties
...
-- Latitude/longitude as doubles with robust casting
ALTER TABLE properties
  ALTER COLUMN latitude TYPE double precision
    USING CASE
      WHEN latitude IS NULL THEN NULL
      WHEN pg_typeof(latitude)::text IN ('double precision','real','numeric','integer','bigint') THEN latitude::double precision
      ELSE NULLIF(regexp_replace(latitude::text, '[^0-9\.\-]+', '', 'g'), '')::double precision
    END,
  ALTER COLUMN longitude TYPE double precision
    USING CASE
      WHEN longitude IS NULL THEN NULL
      WHEN pg_typeof(longitude)::text IN ('double precision','real','numeric','integer','bigint') THEN longitude::double precision
...
-- Spatial backfill with validation and triggers
UPDATE properties
SET geom = CASE
  WHEN latitude IS NULL OR longitude IS NULL THEN NULL
  WHEN latitude < -90 OR latitude > 90 OR longitude < -180 OR longitude > 180 THEN NULL
