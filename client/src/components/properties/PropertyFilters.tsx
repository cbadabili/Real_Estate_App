...
```
Update property type options to use 'plot' instead of 'land'
```
```typescript
import React from 'react';
import { motion } from 'framer-motion';
...
```
The property type options are updated to use 'plot' instead of 'land'.
```
          ...
          <div className="max-h-48 overflow-y-auto space-y-1">
            {botswanaLocations.map((location) => (
              <button
                ...
                className="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-gray-100```diff
--- a/components/PropertyFilters.tsx
+++ b/components/PropertyFilters.tsx
@@ -79,14 +79,13 @@
           <div className="grid grid-cols-2 gap-2">
             {[
               { value: 'all', label: 'All Types' },
...
-              { value: 'land', label: 'Land' }
-            ].map((type) => (
+               { value: 'plot', label: 'Plot' } ].map((type) => (
               <button
                 key={type.value}
                 onClick={() => updateFilter('propertyType', type.value)}
